...
class DeepRMEnv(gym.Env):
    """Custom Environment that follows gym interface"""
    metadata = {'render.modes': ['human']}

    def __init__(self, pa, nw_len_seqs=None, nw_size_seqs=None,
               seed=42, play=False, repre='image', end='all_done',test_mode=False, log_path=None):
        super(DeepRMEnv, self).__init__()
        self.pa = pa
        self.play = play
        ...
        self.observation_space = spaces.Box(low=0, high=1, shape=
                                            (self.pa.network_input_height, self.pa.network_input_width-1),
                                            dtype=np.uint8)
    ...
    def observe(self):
        if self.repre == 'image':
            ...
            backlog_idx = int(self.job_backlog.curr_size / backlog_width)
            image_repr[: backlog_idx,
            ...
            binarized_ob = image_repr[:, 0:-1]
            binarized_ob[binarized_ob > 0] = 1
            # return image_repr
            return binarized_ob
    ...
    def render(self, mode='human', close=False):
        ...
        backlog_idx = int(self.job_backlog.curr_size / backlog_width)
        backlog[: backlog_idx, : backlog_width] = 1
        backlog[backlog_idx, : self.job_backlog.curr_size % backlog_width] = 1
    ...
    def step(self, a, repeat=True):
        ...
        if done:
            ...
            if self.test_mode:
                ...
                # print(self.seq_no+1)
                if self.seq_no+1 == self.pa.num_ex: #maybe wrong
                    if self.log_path is not None:
                        import os
                        assert isinstance(self.log_path, str)
                        self.log_path = os.path.expanduser(self.log_path)
                        os.makedirs(os.path.expanduser(self.log_path), exist_ok=True)
                        record_filename = os.path.join(self.log_path, "test_job_record_rate_{}.npy".format(self.pa.new_job_rate))
                        np.save(record_filename, self.info_seq)
                        exit(0)
            else:
                self.repeat_cnt += 1
                if self.repeat_cnt >= 20:
                # if self.repeat_cnt >= 0:  # Repeat Disabled
                    self.repeat_cnt = 0
                    repeat = False   # automatically use the next sequence if the current sequence has been used more than 20 times
...
def test_backlog():
    ...
    env = DeepRMEnv(pa, play=True, repre='image')
...
def test_compact_speed():
    ...
    env = DeepRMEnv(pa, play=False, repre='compact')

    from baselines.deeprm_env import other_agents
    import time
...
def test_image_speed():
    ...
    env = DeepRMEnv(pa, play=False, repre='image')
